...
import application.control.SimulationAssuranceEmprunt;
...
public class AssuranceEmpruntController { 
	...
	@FXML
	public void actionSimulerAssuranceEmprunt () {
		...
		try {
			...
			if(capitalEmprunt<=0 || dureeEmprunt<=0 || tauxEmprunt<=0 || tauxEmprunt>100 || tauxAssurance<=0 || tauxAssurance>100){
				...
				new SimulationAssuranceEmprunt (primaryStage, dbs, capitalEmprunt, dureeEmprunt, tauxEmprunt, tauxAssurance);
			}
			
		}catch (NumberFormatException nfe){
			Alert alert = new Alert (AlertType.INFORMATION);
			alert.setTitle("Erreur valeurs");
			alert.setContentText("Les valeurs doivent être supérieures à 0\nLes valeurs des taux doivent être supérieures à 0 et inférieures ou égales à 100");
			alert.showAndWait();
		}
	}
